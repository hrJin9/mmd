...
import com.mmd.application.MemberDetailsService;
...
public class JwtTokenProvider {
    ...
    public TokenResponse reissueAccessToken(String email, String authorities) {
        ...
        RefreshToken refreshToken = refreshTokenRepository.findById(email)
                .orElseThrow(() -> new ExpiredTokenException("존재하지 않는 만료된 refresh 토큰입니다."));
        ...
        if(!claims.getSubject().equals(email)) {
            throw new NotValidTokenException("로그인된 사용자의 refresh token이 아닙니다.");
        }
    ...
    public boolean validateToken(String token) {
        try {
            ...
            throw new NotValidTokenException("지원되지 않는 유형의 토큰입니다.");
        } catch (IllegalArgumentException e){
            throw new NotValidTokenException("클레임이 비어있습니다.");
        }
    }
